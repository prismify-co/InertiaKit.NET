...
const routes = [
  ...
  { href: '/users/dashboard', label: 'Insights' },
];
    ...
    <header class="shell-header">
      <div class="container shell-header__inner">
        ...
        <nav class="shell-nav" data-testid="fastendpoints-nav">
          ${routes.map((route) => `
            <a href="${route.href}" class="nav-pill ${currentPath === route.href ? 'nav-pill--active' : ''}" ${currentPath === route.href ? 'aria-current="page"' : ''}>
              ${route.label}
            </a>
          `).join('')}
        </nav>
...
function renderUsersIndex(page) {
  ...
  return `
    ...
    <section class="stats-grid">
      ...
      <div class="stat-card">
        <div class="stat-card__value">${stats?.activeToday ?? '—'}</div>
        <div class="stat-card__label">Active today</div>
      </div>
      <div class="stat-card">
        <div class="stat-card__value">${countries.length}</div>
      ...
      <article>
        ...
        ${users.length > 0 ? `
          <div class="user-rows">
            ${users.map((user) => `
              <div class="user-row">
                <div class="user-row__avatar">${escapeHtml(user.name?.charAt(0) || '?')}</div>
...
function renderDashboard(page) {
  ...
  return `
    ...
    <section class="stats-grid">
      ...
      <div class="stat-card">
        <div class="stat-card__value">${activities.length}</div>
            ...
            ${activities.map((entry) => `
              <div class="list-row">
                ...
                <div class="list-row__aside">
                  <span class="muted-text">${escapeHtml(entry.at)}</span>
                </div>
              </div>
            `).join('')}
          </div>
