...
test('fastendpoints example serves the built-in asset shell in a browser', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1 })).toContainText('Home/Index');

  await page.getByRole('link', { name: 'Users' }).click();
  await expect(page).toHaveURL(/\/users$/);
  await expect(page.getByTestId('fastendpoints-protocol-panel')).toContainText('Protocol features');
  await expect(page.getByTestId('fastendpoints-users-panel')).toContainText('Alice');

  await page.getByRole('link', { name: 'Dashboard' }).click();
  await expect(page).toHaveURL(/\/users\/dashboard$/);
  await expect(page.getByTestId('fastendpoints-dashboard-summary')).toContainText('Dashboard summary');
